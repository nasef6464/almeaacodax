...
- حالة الإنتاج: Vercel `Ready`، وآخر فحص بعد رفع الدليل أثبت أن الإنتاج يخدم commit المتوقع من `main`.
...
## متابعة لاحقة - إغلاق عرض الدورة والدفع للطالب 2026-06-02

- النطاق: صفحة التصنيف، بطاقة الدورة، صفحة الدورة، تبويب الاختبارات، تبويب الملفات، وقفل الاختبارات المدفوعة داخل الدورة.
- النتيجة: `PASS` للعرض والمعاينة والشراء المحمي.
- ما تم إثباته حيًا على الإنتاج:
  - بطاقة الدورة تعرض السعر الحالي والسعر القديم عند وجود خصم، اسم المدرب، عدد الطلاب، زر المعاينة، وزر الشراء.
  - زر `معاينة الدورة` يفتح صفحة الدورة ولا يطلب شراء لمجرد التصفح.
  - زر `شراء الدورة` للزائر يحوله إلى تسجيل الدخول.
  - الاختبارات المقفولة داخل الدورة لا تفتح للزائر وتحوله إلى تسجيل الدخول.
  - تبويب الاختبارات يوضح الفرق بين اختبارات الدورة الرسمية والاختبارات المقترحة من نفس المادة عند عدم وجود ربط رسمي.
  - تبويب الملفات يعرض حالة فارغة نظيفة خاصة بالدورة نفسها ولا يسحب ملفات من مواد أو دورات أخرى.
- فحص الطالب المسجل غير المالك:
  - الحساب: `student.d@almeaa.local`.
  - الدورة: `course_1779224794108`.
  - نافذة الدفع فتحت بنجاح.
  - خطوة طرق الدفع فتحت بنجاح.
  - لم يتم الضغط على `إنشاء طلب شراء` حتى لا نضيف طلب دفع وهمي في الإنتاج.
- الأدلة:
  - `audit-artifacts/ui-audit-exhaustive/2026-06-02-student-course-tabs-live-a52ad041/`
  - `audit-artifacts/ui-audit-exhaustive/2026-06-02-guest-course-purchase-guards-live-8e4ba865/`
  - `audit-artifacts/ui-audit-exhaustive/2026-06-02-student-payment-modal-live-v2-2ab8e5e5/`
- فحص الإنتاج بعد النشر:
  - `npm run smoke:frontend:strict` -> `PASS 29/29`.
  - الإنتاج يخدم commit `2ab8e5e5`.
- الحكم:
  - رحلة معاينة الدورة وشراء الدورة حتى مرحلة اختيار طريقة الدفع صالحة للاستخدام.
  - اعتماد الدفع الفعلي ما زال يعتمد على مراجعة الإدارة أو بوابة الدفع/الويب هوك، ولم يتم تنفيذ اعتماد حقيقي في الفحص حفاظًا على بيانات الإنتاج.
