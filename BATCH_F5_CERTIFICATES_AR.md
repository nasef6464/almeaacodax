...
## تحديث 2026-05-20 (UX Closure)
- إضافة عرض "شهاداتي" داخل الملف الشخصي للطالب:
  - `pages/Profile.tsx`
  - تحميل حي عبر `GET /api/certificates/mine`
  - فتح مباشر لكل شهادة إلى `/certificate/:code`
- إضافة مدخل واضح للشهادات داخل لوحة الطالب:
  - `pages/Dashboard.tsx` (OverviewTab)
  - بطاقة "شهاداتي" بعد بطاقة "المراجعة اليومية" مع عدّاد الشهادات
  - زر مباشر "عرض الشهادات" ينقل إلى `/profile`
- فحوص ما بعد الإضافة:
  - `npm run typecheck` => PASS
  - `npm run build` => PASS
