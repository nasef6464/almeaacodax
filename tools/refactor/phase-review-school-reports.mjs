...
const checks = [
  ...
  ['school reports presentation boundary', 'node', ['scripts/smoke-schools-reports-boundary-contract.mjs']],
  ...
  ['school relationship view-model contract', 'node', ['scripts/smoke-schools-relationship-viewmodel-contract.mjs']],
  ['school workspace view-model contract', 'node', ['scripts/smoke-schools-workspace-viewmodel-contract.mjs']],
  ['school roster contract', 'node', ['scripts/smoke-schools-roster-viewmodel-contract.mjs']],
  ['school package/access contract', 'node', ['scripts/smoke-schools-package-access-viewmodel-contract.mjs']],
...
for (const [name, command, args] of checks) {
  ...
  console.log(`\n[reports-phase-review] START ${name}`);
  ...
  if (result.status !== 0) {
    console.error(`\n[reports-phase-review] FAIL ${name}`);
    console.error(JSON.stringify(results, null, 2));
    process.exit(result.status ?? 1);
  }
}

console.log('\n[reports-phase-review] ALL CHECKS PASS');
