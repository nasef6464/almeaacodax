...
const apply = spawnSync('node', ['tools/refactor/apply-results-score-presentation.mjs'], {
  stdio: 'inherit',
  shell: process.platform === 'win32',
});
if (apply.status !== 0) {
  console.error('[results-score-presentation-phase-review] Apply/confirm failed');
...
const checks = [
  ...
  ['Results score presentation boundary', 'node', ['scripts/smoke-results-score-presentation-boundary-contract.mjs', '--require-post']],
  ['Results product contract', 'node', ['scripts/smoke-results-contract.mjs']],
...
for (const [name, command, args] of checks) {
  ...
  console.log(`\n[results-score-presentation-phase-review] START ${name}`);
  const result = spawnSync(command, args, { stdio: 'inherit', shell: process.platform === 'win32' });
  results.push({ name, status: result.status === 0 ? 'PASS' : 'FAIL', durationMs: Date.now() - startedAt });
  if (result.status !== 0) {
    console.error(`\n[results-score-presentation-phase-review] FAIL ${name}`);
    console.error(JSON.stringify(results, null, 2));
    process.exit(result.status ?? 1);
  }
}

const allowedFiles = new Set(['pages/Results.tsx']);
...
if (changed.status !== 0 || untracked.status !== 0) {
  console.error('[results-score-presentation-phase-review] Failed to inspect working-tree scope');
  process.exit(1);
}
const changedFiles = [...new Set(`${changed.stdout || ''}\n${untracked.stdout || ''}`.split(/\r?\n/).map((item) => item.trim()).filter(Boolean))];

if (changedFiles.length === 0) {
  console.log('\n[results-score-presentation-phase-review] ALL CHECKS PASS — verified clean no-op');
...
if (unexpected.length) {
  console.error(`[results-score-presentation-phase-review] Unexpected changed files: ${unexpected.join(', ')}`);
  process.exit(1);
}
for (const required of allowedFiles) {
  if (!changedFiles.includes(required)) {
    console.error(`[results-score-presentation-phase-review] Expected verified change missing: ${required}`);
    process.exit(1);
  }
}
...
if (stage.status !== 0) {
  console.error('[results-score-presentation-phase-review] Failed to stage verified files');
  process.exit(stage.status ?? 1);
}

console.log('\n[results-score-presentation-phase-review] ALL CHECKS PASS');
