...
const checks = [
  ...
  ['school command-center presentation boundary', 'node', ['scripts/smoke-schools-command-center-boundary-contract.mjs']],
  ['school overview operations presentation boundary', 'node', ['scripts/smoke-schools-overview-operations-boundary-contract.mjs']],
  ['school overview operator presentation boundary', 'node', ['scripts/smoke-schools-overview-operators-boundary-contract.mjs']],
...
for (const [name, command, args] of checks) {
  ...
  console.log(`\n[command-center-phase-review] START ${name}`);
  ...
  if (result.status !== 0) {
    console.error(`\n[command-center-phase-review] FAIL ${name}`);
    console.error(JSON.stringify(results, null, 2));
    process.exit(result.status ?? 1);
  }
}

console.log('\n[command-center-phase-review] ALL CHECKS PASS');
