...
for (const [name, command, args] of checks) {
  ...
  console.log(`\n[reports-directed-quiz-analytics-phase-review] START ${name}`);
  ...
  if (result.status !== 0) {
    console.error(`\n[reports-directed-quiz-analytics-phase-review] FAIL ${name}`);
    console.error(JSON.stringify(results, null, 2));
    process.exit(result.status ?? 1);
  }
}

console.log('\n[reports-directed-quiz-analytics-phase-review] ALL CHECKS PASS');
