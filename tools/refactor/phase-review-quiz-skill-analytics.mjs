...
const applied = spawnSync('node', ['tools/refactor/apply-quiz-skill-analytics.mjs'], {
  stdio: 'inherit',
  shell: process.platform === 'win32',
});
if (applied.status !== 0) {
  console.error('[quiz-skill-analytics-phase-review] Failed to apply/confirm current phase');
...
if (stageVerifiedFiles.status !== 0) {
  console.error('[quiz-skill-analytics-phase-review] Failed to stage verified route changes');
  process.exit(stageVerifiedFiles.status ?? 1);
}

console.log('\n[quiz-skill-analytics-phase-review] ALL CHECKS PASS');
