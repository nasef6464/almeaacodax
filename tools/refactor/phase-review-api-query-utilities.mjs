...
const apply = spawnSync('node', ['tools/refactor/apply-api-query-utilities.mjs'], {
  stdio: 'inherit',
  shell: process.platform === 'win32',
});
if (apply.status !== 0) {
  console.error('[api-query-utilities-phase-review] Apply/confirm failed');
...
const checks = [
  ...
  ['API query utilities boundary', 'node', ['scripts/smoke-api-query-utilities-boundary-contract.mjs']],
  ['API phase 4 compatibility contract', 'npm', ['run', 'smoke:api-phase4']],
  ['Frontend phase 5 compatibility contract', 'npm', ['run', 'smoke:frontend-phase5']],
  ['CSRF contract', 'npm', ['run', 'smoke:csrf']],
  ['Auth cookie contract', 'npm', ['run', 'smoke:auth-cookie']],
...
if (changed.status !== 0 || untracked.status !== 0) {
  console.error('[api-query-utilities-phase-review] Failed to inspect working-tree scope');
  process.exit(1);
}
const changedFiles = [...new Set(`${changed.stdout || ''}\n${untracked.stdout || ''}`.split(/\r?\n/).map((item) => item.trim()).filter(Boolean))];

if (changedFiles.length === 0) {
  console.log('\n[api-query-utilities-phase-review] ALL CHECKS PASS — verified clean no-op');
...
if (unexpected.length) {
  console.error(`[api-query-utilities-phase-review] Unexpected changed files: ${unexpected.join(', ')}`);
  process.exit(1);
}
for (const required of allowedFiles) {
  if (!changedFiles.includes(required)) {
    console.error(`[api-query-utilities-phase-review] Expected verified change missing: ${required}`);
    process.exit(1);
  }
}
...
if (stageVerifiedFiles.status !== 0) {
  console.error('[api-query-utilities-phase-review] Failed to stage verified files');
  process.exit(stageVerifiedFiles.status ?? 1);
}

console.log('\n[api-query-utilities-phase-review] ALL CHECKS PASS');
