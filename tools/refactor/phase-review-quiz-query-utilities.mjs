...
const applied = spawnSync('node', ['tools/refactor/apply-quiz-query-utilities.mjs'], {
  stdio: 'inherit',
  shell: process.platform === 'win32',
});
if (applied.status !== 0) {
  console.error('[quiz-query-utilities-phase-review] Failed to apply/confirm current phase');
...
const checks = [
  ...
  ['quiz submission schema boundary', 'node', ['scripts/smoke-quiz-submission-schema-boundary-contract.mjs']],
  ['quiz question presentation boundary', 'node', ['scripts/smoke-quiz-question-presentation-boundary-contract.mjs']],
  ['quiz skill analytics boundary', 'node', ['scripts/smoke-quiz-skill-analytics-boundary-contract.mjs']],
  ['quiz query utilities boundary', 'node', ['scripts/smoke-quiz-query-utilities-boundary-contract.mjs']],
  ['question bank runtime CRUD contract', 'node', ['scripts/smoke-batch100p-question-bank-runtime-crud-contract.mjs']],
...
if (stageVerifiedFiles.status !== 0) {
  console.error('[quiz-query-utilities-phase-review] Failed to stage verified route changes');
  process.exit(stageVerifiedFiles.status ?? 1);
}

console.log('\n[quiz-query-utilities-phase-review] ALL CHECKS PASS');
