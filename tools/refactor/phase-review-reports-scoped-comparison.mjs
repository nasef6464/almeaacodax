...
for (const [name, command, args] of checks) {
  ...
  console.log(`\n[reports-scoped-comparison-phase-review] START ${name}`);
  ...
  if (result.status !== 0) {
    console.error(`\n[reports-scoped-comparison-phase-review] FAIL ${name}`);
    console.error(JSON.stringify(results, null, 2));
    process.exit(result.status ?? 1);
  }
}

console.log('\n[reports-scoped-comparison-phase-review] ALL CHECKS PASS');
