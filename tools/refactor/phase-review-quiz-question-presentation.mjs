...
const applied = spawnSync('node', ['tools/refactor/apply-quiz-question-presentation.mjs'], {
  stdio: 'inherit',
  shell: process.platform === 'win32',
});
if (applied.status !== 0) {
  console.error('[quiz-question-presentation-phase-review] Failed to apply/confirm current phase');
...
for (const [name, command, args] of checks) {
  ...
  console.log(`\n[quiz-question-presentation-phase-review] START ${name}`);
  const result = spawnSync(command, args, { stdio: 'inherit', shell: process.platform === 'win32' });
  results.push({ name, status: result.status === 0 ? 'PASS' : 'FAIL', durationMs: Date.now() - startedAt });
  if (result.status !== 0) {
    console.error(`\n[quiz-question-presentation-phase-review] FAIL ${name}`);
...
if (stageVerifiedFiles.status !== 0) {
  console.error('[quiz-question-presentation-phase-review] Failed to stage verified route changes');
  process.exit(stageVerifiedFiles.status ?? 1);
}

console.log('\n[quiz-question-presentation-phase-review] ALL CHECKS PASS');
