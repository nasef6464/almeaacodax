...
async function inspectRoute(page, name, route, expectations = {}) {
  ...
  const state = await page.evaluate(({ expectedNextActions, requiredSelectors, actionGroupBudgets, nextActionStrip, quizStepHint }) => {
    ...
    const stripStatus = stripProbe
      ...
      : true;
    const quizHint = document.querySelector('[data-testid="quiz-current-step-hint"]');
    const quizHintText = quizHint?.textContent?.trim() || "";
    const quizHintStatus = quizStepHint
      ? Boolean(quizHint) &&
        quizHintText.length >= 20 &&
        quizHintText.length <= 160 &&
        /أجب|التالي|إنهاء/.test(quizHintText)
      : true;
    return {
      href: location.href,
      ...
      quizStepHintProbe: quizStepHint ? { exists: Boolean(quizHint), text: quizHintText } : null,
      quizStepHintStatus: quizHintStatus ? "PASS" : "FAIL",
    ...
    quizStepHint: Boolean(expectations.quizStepHint),
  });
...
try {
  const routePlan = [
    ...
    ["subject-skill-map", targets.routes.subject, { minBodyLength: 500, minControls: 8, nextActionStrip: true, requiredSelectors: ['[data-testid="student-next-action-strip"]', '[data-testid="student-next-action-primary"]'], nextActions: ["افتح", "ابدأ", "تدريب", "اختبار", "عرض الباقات"] }],
