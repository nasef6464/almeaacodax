...
check('realtime reconnect rejoins the room and refreshes canonical state', realtime.includes("socket.on('connect', () => {") && realtime.includes('sessionSubscribers.forEach((_, id) => joinSession(id))') && realtime.includes("notifySession(id, 'connected')"));
