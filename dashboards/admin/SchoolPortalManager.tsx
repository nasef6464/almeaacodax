...
interface SchoolPortalManagerProps {
    onOpenSchoolOperations?: () => void;
}
...
export const SchoolPortalManager: React.FC<SchoolPortalManagerProps> = ({ onOpenSchoolOperations }) => {
    const {
        user,
        users,
        groups,
    ...
    const [alertingStudentId, setAlertingStudentId] = useState('');
    ...
    const directedQuizSnapshots = useMemo(() => (
        reportFollowUpQuizzes
            .map((quiz) => {
                const quizResults = scope.results.filter((result) => result.quizId === quiz.id && !!result.userId && reportStudentIds.has(result.userId));
                return {
                    quiz,
                    attempts: quizResults.length,
                    average: averageScore(quizResults),
                    targetCount: (quiz.targetUserIds || []).filter((studentId) => reportStudentIds.has(studentId)).length
                        + (quiz.targetGroupIds || []).filter((groupId) => reportClasses.some((classroom) => classroom.id === groupId)).length,
                };
            })
            .sort((a, b) => b.quiz.createdAt - a.quiz.createdAt)
            .slice(0, 5)
    ), [reportClasses, reportFollowUpQuizzes, reportStudentIds, scope.results]);
    ...
    const openSchoolOperations = () => {
        if (user.role !== Role.ADMIN) {
            setActionFeedback('تشغيل المدارس متاح لمدير المنصة فقط. هذه البوابة مخصصة لمتابعة طلابك وفصولك وإرسال التنبيهات والتقارير.');
            return;
        }
        if (onOpenSchoolOperations) {
            onOpenSchoolOperations();
            setActionFeedback('تم فتح تشغيل المدارس من داخل لوحة الإدارة.');
            return;
        }
        window.location.hash = '/admin-dashboard?tab=schools';
        setActionFeedback('تم فتح تشغيل المدارس من داخل لوحة الإدارة.');
    };
    ...
    const buildStudentAlertBody = (summaries: typeof reportStudentSummaries) => {
        const firstWeakSkill = summaries.flatMap((summary) => summary.weakSkills).sort((a, b) => a.mastery - b.mastery)[0]?.skill;
        return [
            `تنبيه متابعة من ${schoolTitle}`,
            summaries.length === 1 ? `الطالب: ${summaries[0].student.name}` : `عدد الطلاب: ${summaries.length}`,
            firstWeakSkill ? `المهارة التي تحتاج تركيزًا: ${firstWeakSkill}` : 'المطلوب الآن: دخول المنصة وحل اختبار المتابعة القصير.',
            'ادخل إلى حسابك وابدأ من الاختبار أو التدريب الموجه لك.',
        ].join('\n');
    };

    const sendPriorityStudentAlerts = async () => {
        const targets = watchList.slice(0, 20);
        if (!targets.length) {
            setActionFeedback('لا توجد قائمة متابعة تحتاج تنبيهًا الآن.');
            return;
        }
        ...
        try {
            await api.sendStudentAlert({
                studentIds: targets.map((summary) => summary.student.id),
                title: 'تنبيه متابعة دراسية',
                body: buildStudentAlertBody(targets),
                channels: ['in_app'],
            });
            setActionFeedback(`تم إرسال تنبيه داخل المنصة إلى ${targets.length} طالب داخل نطاقك.`);
        } catch {
            setActionFeedback('تعذر إرسال التنبيه الآن. تأكد من اتصال الخادم أو صلاحية نطاق المشرف.');
        } finally {
            setAlertSending(false);
        }
    };

    const sendStudentFollowUpAlert = async (summary: typeof reportStudentSummaries[number]) => {
        setAlertingStudentId(summary.student.id);
        try {
            await api.sendStudentAlert({
                studentIds: [summary.student.id],
                title: 'تنبيه متابعة دراسية',
                body: buildStudentAlertBody([summary]),
                channels: ['in_app'],
            });
            setActionFeedback(`تم إرسال تنبيه داخل المنصة إلى ${summary.student.name}.`);
        } catch {
            setActionFeedback('تعذر إرسال التنبيه لهذا الطالب. قد يكون خارج نطاق المشرف أو حدث خطأ في الاتصال.');
        } finally {
            setAlertingStudentId('');
        }
    };
            ...
            <div className="rounded-2xl border border-indigo-100 bg-white p-5 shadow-sm">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:justify-end">
                        ...
                        <button
                            type="button"
                            onClick={sendPriorityStudentAlerts}
                            disabled={alertSending || watchList.length === 0}
                            data-testid="supervisor-send-priority-alerts"
                            className="inline-flex items-center justify-center gap-2 rounded-xl bg-rose-50 px-4 py-2.5 text-xs font-black text-rose-700 hover:bg-rose-100 disabled:cursor-not-allowed disabled:opacity-50"
                        >
                            <Bell size={15} />
                            {alertSending ? 'جار الإرسال...' : 'تنبيه الأولوية'}
                        </button>
                        <button
                            type="button"
                ...
                <div data-testid="supervisor-issued-tests-panel" className="mt-4 rounded-2xl border border-indigo-100 bg-white p-4">
                    <div className="mb-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                        <div>
                            <h3 className="text-base font-black text-gray-900">اختباراتي الموجهة ونتائجها</h3>
                            <p className="mt-1 text-xs font-bold text-gray-500">آخر اختبارات داخل نطاقك مع عدد المحاولات ومتوسط الدرجات.</p>
                        </div>
                        <button
                            type="button"
                            onClick={openTargetedQuiz}
                    ...
                    <div className="grid gap-3 lg:grid-cols-2">
                        {directedQuizSnapshots.length === 0 ? (
                            <div className="rounded-xl border border-dashed border-gray-200 p-4 text-sm font-bold text-gray-500">
                                لم تظهر اختبارات موجهة داخل نطاقك بعد. ابدأ باختبار قصير للفصل أو الطلاب المحددين.
                            </div>
                        ) : directedQuizSnapshots.map((item) => (
                            <div key={item.quiz.id} className="rounded-xl border border-gray-100 bg-gray-50/70 p-4">
                                <div className="flex items-start justify-between gap-3">
                                    <div>
                                        <div className="text-sm font-black text-gray-900">{item.quiz.title}</div>
                                        <div className="mt-1 text-xs font-bold text-gray-500">
                                            {item.targetCount || 'نطاق'} مستهدف · {item.attempts} محاولة
                                        </div>
                                    </div>
                                    <span className={`rounded-full px-3 py-1 text-xs font-black ${
                                        ...
                                        {item.attempts ? `${item.average}%` : 'بدون نتائج'}
                                    </span>
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-right text-sm">
                            ...
                            <tbody>
                                {watchList.length === 0 ? (
                                    <tr>
                                        <td colSpan={6} className="py-8 text-center text-sm font-bold text-emerald-700">
                                    ...
                                    <tr key={summary.student.id} className="border-b border-gray-50 last:border-0">
                                        ...
                                        <td className="py-3">
                                            <button
                                                type="button"
                                                onClick={() => void sendStudentFollowUpAlert(summary)}
                                                disabled={alertingStudentId === summary.student.id}
                                                data-testid="supervisor-send-student-alert"
                                                className="inline-flex items-center gap-1.5 rounded-lg bg-rose-50 px-3 py-1.5 text-xs font-black text-rose-700 hover:bg-rose-100 disabled:cursor-not-allowed disabled:opacity-50"
                                            >
                                                <Bell size={13} />
                                                {alertingStudentId === summary.student.id ? 'جار الإرسال' : 'تنبيه'}
                                            </button>
                                        </td>
                                    </tr>
                                ))}
