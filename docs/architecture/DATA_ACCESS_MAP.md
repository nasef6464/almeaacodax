...
| Users/Groups/Memberships | آلاف/مئات المدارس | scope/roster/report؛ School Teacher workspace؛ interactive-video resume | `schoolTeacherWorkspace` يجمع memberships النشطة ثم assignments محدودة بالمدارس، ويتحقق من `CLASS.parentId` قبل عرض الفصل، ويقرأ حتى 100 اختبار منشور موجه للفصول الصالحة؛ Teacher لا يملك Group CRUD؛ Admin relations import أو بطاقة الفصل تسند Teacher عبر schoolId/groupIds دون supervisorIds؛ قائمة `interactiveVideoProgress` شخصية ومحدودة بـ100 سجل/100 سؤال لكل سجل عبر `/auth/me/preferences` | PARTIAL — G8 isolated HTTP/UI scope proven; production scale NOT PROVEN |
