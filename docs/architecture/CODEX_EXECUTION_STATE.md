...
- Current Goal: `G2 — Usable Smart Classroom` (`SC-02A + SC-02B`) is `IN PROGRESS`; G1 is `CLOSED / VERIFIED` and G0 is `CLOSED / VERIFIED WITH FOCUSED CI`.
...
- G2 evidence update: exact runtime `d57c1376021fc2bc73385fa52c312f880f96384b` passed Backend Integration [34394732870](https://github.com/nasef6464/almeaacodax/actions/runs/34394732870), including an isolated real-HTTP School A/B lifecycle: contract and teacher assignment, PIN join rejection for outsider, safe question projection, valid-range answer handling, idempotency, and immutable end report. Local Playwright surfaces audit passed for Teacher Console, Student Live View, and Projector aggregate/no-identity rendering. A real live-Socket reconnect observation remains `NOT PROVEN`; G2 is still open for that narrow evidence gap.
