...
- Current frontend delivery: Student dashboard visual button system and colorful navigation badges unified across sidebar items, ExamsHubTab, MockExamStudentHub, and Quizzes attempt tabs. Active buttons feature vibrant distinct states with drop shadows; inactive buttons feature solid slate-100/border-slate-200 tactile styling; button sizing is balanced and proportional (w-fit/inline-flex) resolving unclickable text perceptions. All smoke contracts (10/10 quizzes, 13/13 student journey) and production build pass.
