...
- Current batch: assessment isolated HTTP integration coverage added
- Current branch: `refactor/modular-platform-safe`
- Last completed commit: `55a8548a` (directed assessment HTTP journey); legacy-builder inventory guard is `645cff42`
...
- Next exact action: run the assessment HTTP journey in its isolated CI Mongo environment; then add supervisor/teacher scope cases only after seeding their real school/group topology. Do not delete a builder or change routes/schema/RBAC/scoring.
