...
## Deep Modularity / Authority / Resource Audit — 2026-09-19

- Status: COMPLETE repository-wide architecture audit checkpoint on branch `fix/parent-authority-b9`; this is an audit checkpoint, not Batch 9 closure.
- Current tree evidence: 1,466 files. Current filesystem/runtime ownership is canonicalized in `docs/architecture/CURRENT_DIRECTORY_AND_MODULE_MAP.md`.
- Deep findings and batch ownership are canonicalized in `docs/architecture/DEEP_MODULARITY_AND_RESOURCE_AUDIT.md`.
- Execution rule changed: from Batch 9 onward each owning domain completes worthwhile modular extraction while correctness/authority is already under inspection. Batch 14 is residual sweep only; no second wholesale refactor pass is planned.
- Strong current boundaries to preserve: explicit server bootstrap/shutdown; Smart Classroom route registrars; assessment application/infrastructure extraction; school application policies; Redis/SSE notification realtime; BullMQ weekly scheduler; frontend SchoolsManager/Reports/API-group/slice extraction patterns.
- High-change-radius backend hotspots: `content.routes.ts`, `quiz.routes.ts`, `payment.routes.ts`, `auth.routes.ts`, `ai.routes.ts`, then notification/public-test/operations facades.
- High-change-radius frontend hotspots: `Reports.tsx`, `QuizPage.tsx`, `PathsManager.tsx`, `Dashboard.tsx`, `FinancialManager.tsx`, `AdminDashboard.tsx`, `QuestionBankManager.tsx`, `store/useStore.ts`, and `App.tsx`. Existing feature extraction must be continued rather than restarted.
- Frontend target-state correction: `src/app|core|features|shared` is still a target. Current runtime source remains primarily root `pages/components/dashboards/services/store/utils`; `src/` currently contains only observability. Do not bulk-move files to mimic the target.
- Bandwidth/media evidence: normal learning media is URL/CDN-reference based; Node is not the ordinary binary media CDN and no standard first-party upload runtime is exposed. Cloudflare-backed delivery is the target operational boundary; live Cloudflare configuration/cache/egress remains LIVE VERIFICATION REQUIRED. Batch 13 must measure API bytes/request counts/bootstrap duplication/origin egress/cache behavior.
- Data-access findings: scoped/bootstrap caching and in-flight dedupe exist, but some scoped content/admin/operations reads remain unbounded by pagination and require measured scale remediation rather than intuition.
- Privacy finding: current user deletion is partial; AI interactions, client events, audit logs, attempts and other user-linked collections have no complete retention/anonymization orchestration. Batch 10 starts with an explicit retention matrix.
- Backup finding: learning snapshots and Mongo dump/restore scripts exist, but full DR proof (off-site, encryption/access, rotation, checksum, restore drill, RPO/RTO) is not established. Batch 11 owns it.
- Batch 9 CRITICAL findings before merge: canonical parent revoked rows currently can fail open to legacy fallback; intervention parent recipients have the same tombstone issue; parent→non-parent role transition/upsert can leave stale authority; admin weekly WhatsApp trigger still uses legacy/N+1 reads; weekly batch still has per-parent authority/idempotency queries; notification audiences still use legacy teacher/supervisor group fields; >500-recipient notification requests silently truncate. These must be fixed/protected before Batch 9 is DONE.
