...
## 18) اعتماد B1/B2 — تجربة الجوال للأسئلة الكمية والمعلّم الصوتي

اعتمدت بعد مراجعة لقطات الهاتف الفعلية:

### B1 — السؤال الكمي المصوّر
- إذا كان `optionsEmbeddedInImage=true` ومع السؤال `imageUrl`، تكون الصورة هي العرض الأساسي لأنها تحتوي نص السؤال والاختيارات.
- لا يكرر الـUI نص السؤال أو نصوص الاختيارات أسفل الصورة.
- أزرار الإجابة التفاعلية تعرض فقط: **أ / ب / ج / د**.
- الربط الداخلي يبقى بالـindex: أ=0، ب=1، ج=2، د=3، ويظل `correctOptionIndex` مصدر التصحيح.
- أي Option قديم/غير سليم يصل ككائن لا يجوز عرضه كـ`[object Object]`; يتم تطبيعه أو استبداله بحرف الاختيار المناسب.
- لا تنشأ نسخة جديدة من الصورة أو السؤال لهذا العرض.
- يطبق العقد نفسه على محرك الاختبار الذاتي `/quiz` ومحرك الاختبار المحضّر `/quiz/:quizId`؛ لا يوجد مساران بصريان مختلفان للأسئلة المصورة.

### B2 — المعلّم الذكي في مراجعة السؤال
- واجهة الطالب **صوتية فقط**؛ لا حقل كتابة ولا أزرار تلميح/شرح ثابتة ولا كتلة «ناقش هذا السؤال».
- يظهر زر ميكروفون صغير عند الطلب.
- صوت الطالب -> STT قصير -> نفس Question Assistant/Gateway -> رد نصي داخلي -> TTS صوتي.
- لا يرسل Image bytes للـAI افتراضيًا، ولا يغير الدرجة أو الإتقان.
- يبقى الحوار scoped إلى `questionId` والسياق المسموح.
- لا يُنشأ Tutor جديد؛ الواجهة الصوتية Adapter فوق نفس Question Assistant.

### Mobile density
- تقليل Padding وارتفاع البطاقات في Quiz/Results/Review.
- لوحة الأسئلة تكون قابلة للفتح على الجوال بدل كارت دائم طويل.
- ملخص النتيجة يضع الدرجة/الصحيح/الخطأ/الوقت في أعلى الشاشة بصورة مضغوطة، والتفاصيل الأعمق تظل أسفلها.
- مراجعة السؤال تعرض الصورة ثم أزرار أ/ب/ج/د ثم المعلّم الصوتي والتنقل، بأقل Scroll ممكن.
