...
export const SmartClassroomExamRunner: React.FC<SmartClassroomExamRunnerProps> = ({
  questions, initialIndex = 0, durationMinutes = 10, isChallenge = false,
  answers, onSelectAnswer, onSubmit, submitted, submitting, message,
}) => {
  const [currentIndex, setCurrentIndex] = useState(initialIndex);
  const [timeLeft, setTimeLeft] = useState(durationMinutes * 60);

  // Sync index if external index changes
  useEffect(() => {
  ...
  return (
    <div
      className="fixed inset-0 z-[9999] flex flex-col bg-slate-950/95 backdrop-blur-xl text-white select-none overflow-y-auto"
      dir="rtl"
    >
      ...
      <header className="sticky top-0 z-20 border-b border-slate-800 bg-slate-900/90 px-4 sm:px-6 py-3.5 backdrop-blur-md">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div
              className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl ${
                isChallenge ? 'bg-amber-500/20 text-amber-400' : 'bg-indigo-500/20 text-indigo-400'
              }`}
            >
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-sm sm:text-base lg:text-lg font-black text-white">
                  الحصة الذكية · تدريب الفصل المباشر
                </h2>
                <span
                  className={`rounded-full px-2.5 py-0.5 text-xs font-black ${
                    isChallenge ? 'bg-amber-500 text-slate-950' : 'bg-indigo-500 text-white'
                  }`}
                >
          ...
          <div className="flex items-center gap-3">
            ...
            <div
              className={`flex items-center gap-2 rounded-2xl border px-3.5 py-1.5 font-mono text-sm sm:text-base font-black transition-all ${
                isTimeCritical
            ...
            <div className="hidden md:flex items-center gap-1.5 text-slate-400 bg-slate-800/80 px-3 py-1.5 rounded-xl text-xs font-bold">
              ...
              <span>شاشة مغلقة</span>
            </div>
          </div>
        </div>
      </header>

      {/* Main Container: Split Layout with Left Side Question Map */}
      <div className="mx-auto flex max-w-7xl w-full flex-1 flex-col lg:flex-row gap-6 p-4 sm:p-6 lg:p-8">
        {/* Left Side Question Map (أرقام الأسئلة على الجانب اليسار) */}
          ...
          <div className="rounded-3xl border border-slate-800 bg-slate-900/80 p-5 shadow-xl backdrop-blur-md">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h3 className="text-sm font-black text-white flex items-center gap-2">
                <Flag size={16} className="text-indigo-400" /> خريطة الأسئلة
              </h3>
              <span className="text-xs font-mono font-bold text-slate-400">
                {answeredCount} / {totalQuestions} محلول
              </span>
            </div>
            ...
            {/* Question Numbers Grid: [1] [2] [3] ... [10] */}
              ...
              {questions.map((_, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`relative flex h-11 w-full items-center justify-center rounded-xl text-sm font-black transition-all cursor-pointer ${
                      isCurrent
                        ? 'bg-indigo-600 text-white shadow-lg ring-4 ring-indigo-500/30 scale-105 z-10'
                        : isAnswered
                        ? 'border border-emerald-500/40 bg-emerald-950/60 text-emerald-300 hover:bg-emerald-900/60'
                        : 'border border-slate-800 bg-slate-800/60 text-slate-300 hover:border-slate-700 hover:bg-slate-700'
                    }`}
                  >
                    {idx + 1}
                    {isAnswered && !isCurrent && (
                      <span className="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-emerald-500 text-[10px] text-slate-950">
            ...
            <div className="mt-5 border-t border-slate-800 pt-4">
              <button
                ...
                onClick={onSubmit}
                className="w-full flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 py-3 text-sm font-black text-white shadow-lg transition-all hover:from-emerald-700 hover:to-teal-700 disabled:opacity-50"
              >
                <Send size={16} />
                {submitted ? 'تم تسليم الاختبار' : 'تسليم وإنهاء الاختبار'}
              </button>
            </div>
          </div>
        </aside>

        {/* Center Main: Question Stem, Tablet Options & Navigation (منطقة السؤال والتنقل) */}
          ...
          {submitted ? (
            <div className="flex-1 flex items-center justify-center">
              <div className="w-full max-w-xl rounded-3xl border border-emerald-500/30 bg-slate-900/90 p-8 text-center shadow-2xl backdrop-blur-md">
                ...
                <p className="mt-2 text-sm text-slate-300">
                  تم تسجيل إجابة <span className="font-bold text-emerald-400">{answeredCount}</span> من أصل{' '}
                  <span className="font-bold text-white">{totalQuestions}</span> أسئلة على رادار الحصة المباشر.
                </p>
                <div className="mt-6 inline-flex items-center gap-2 rounded-2xl bg-emerald-950/60 border border-emerald-500/40 px-5 py-2.5 text-sm font-bold text-emerald-300">
                  <span>انتظر المعلم لمناقشة النتائج على السبورة التفاعلية 🌟</span>
                </div>
              </div>
            </div>
          ) : !currentQ ? (
            <div className="flex-1 flex items-center justify-center p-12 text-center">
              <div className="rounded-3xl border border-slate-800 bg-slate-900/60 p-8">
                ...
                <p className="text-slate-300">بانتظار المعلم لنشر الأسئلة…</p>
              </div>
            </div>
          ) : (
            <div className="flex flex-col flex-1 justify-between rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 sm:p-8 lg:p-10 shadow-2xl">
              ...
              <div>
                <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-5">
                  <div className="flex items-center gap-2.5">
                    <span className="inline-flex items-center gap-2 rounded-xl bg-indigo-50 px-3.5 py-1.5 text-xs font-black text-indigo-700 dark:bg-indigo-950/60 dark:text-indigo-300">
                      <span className="h-2 w-2 rounded-full bg-indigo-600 animate-pulse" />
                      السؤال {currentIndex + 1} من {totalQuestions}
                    </span>
                    {isChallenge && (
                      <span className="inline-flex items-center gap-1 rounded-xl bg-amber-100 px-3 py-1 text-xs font-black text-amber-800 dark:bg-amber-950/50 dark:text-amber-300">
                        ⚡ تحدي السرعة
                      </span>
                    )}
                  </div>
                  ...
                  >
                    {answers[currentIndex] !== undefined ? '✓ تمت الإجابة' : 'لم تجب بعد'}
                  </span>
                </div>
                ...
                {/* Optional Image */}
                {currentQ.imageUrl && (
                  <div className="mt-4 overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-2">
                    <img
                  ...
                  {currentQ.options.map((opt: string, optIdx: number) => {
                    ...
                    return (
                      <button
                        ...
                        className={`group flex min-h-[76px] sm:min-h-[88px] items-center justify-between rounded-2xl border-2 p-4 sm:p-5 text-right transition-all cursor-pointer ${
                          isSelected
                      ...
                      >
                        <div className="flex items-center gap-3.5 flex-1 min-w-0">
                          <span
                            className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl text-base font-black transition-all ${
                              isSelected
              ...
              {/* Navigation Bar (يمين ويسار: السابق / التالي) */}
              <div className="mt-10 pt-6 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-4">
                <button
                  type="button"
                  disabled={currentIndex === 0}
                  onClick={handlePrev}
                  className="flex items-center gap-2 rounded-2xl border border-slate-300 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 px-5 sm:px-6 py-3.5 text-sm sm:text-base font-black text-slate-800 dark:text-slate-200 transition-all hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  <ArrowRight size={20} />
                  السابق
                </button>
                ...
                <button
                  type="button"
                  onClick={handleNext}
                  className="flex items-center gap-2 rounded-2xl bg-indigo-600 px-6 sm:px-8 py-3.5 text-sm sm:text-base font-black text-white shadow-lg transition-all hover:bg-indigo-700 active:scale-95"
                >
                  {currentIndex === totalQuestions - 1 ? 'تسليم الاختبار' : 'التالي'}
                  <ArrowLeft size={20} />
                </button>
              </div>

              {message && (
                <p className="mt-3 text-center text-xs font-bold text-slate-500">{message}</p>
              )}
            </div>
          )}
        </main>
