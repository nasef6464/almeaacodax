...
const allowedQuestionImageTypes = new Map([
  ["image/jpeg", "jpg"],
  ["image/png", "png"],
  ["image/webp", "webp"],
]);

type CreateQuestionImageUploadIntentInput = {
  contentType: string;
  sizeBytes: number;
};

export type QuestionImageUploadIntent = {
  uploadUrl: string;
  publicUrl: string;
  key: string;
  method: "PUT";
...
export function createQuestionImageUploadIntent({
  contentType,
  sizeBytes,
}: CreateQuestionImageUploadIntentInput): QuestionImageUploadIntent {
  requireUploadConfiguration();

  const normalizedContentType = String(contentType || "").trim().toLowerCase();
  const extension = allowedQuestionImageTypes.get(normalizedContentType);
  if (!extension) {
    const error = new Error("Only JPEG, PNG and WebP question images are allowed") as Error & { statusCode?: number };
  ...
  const key = `questions/editor/${year}/${month}/${randomUUID()}.${extension}`;
