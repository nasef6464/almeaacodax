...
const interventionSchema = z.object({
  ...
  targetStudentIds: z.array(z.string().min(1)).min(1).max(50), pathId: z.string().min(1),
  dailyMinutes: z.number().int().min(15).max(240).optional().default(90), followUpAt: z.string().datetime().optional(),
  remediationThreshold: z.number().min(0).max(100).optional(), minimumEvidence: z.number().int().min(1).max(500).optional(),
});
const idQuery = (ids: string[]) => ({ $or: [
  { id: { $in: ids } },
  ...(ids.some((id) => Types.ObjectId.isValid(id)) ? [{ _id: { $in: ids.filter((id) => Types.ObjectId.isValid(id)) } }] : []),
] });
...
export function registerClassroomSupervisorRoutes(classroomRouter: Router) {
  ...
  classroomRouter.get("/supervisor/teachers", requireAuth, requireRole(["admin", "supervisor"]), asyncHandler(async (req, res) => {
    res.json({ teachers: await buildClassroomTeacherReports(await resolveClassroomSupervisorScope(req.authUser!)) });
  }));
  classroomRouter.get("/supervisor/intelligence", requireAuth, requireRole(["admin", "supervisor"]), asyncHandler(async (req, res) => {
    res.json({ intelligence: await buildClassroomSchoolIntelligence(await resolveClassroomSupervisorScope(req.authUser!)) });
  }));
  classroomRouter.get("/supervisor/interventions", requireAuth, requireRole(["admin", "supervisor"]), asyncHandler(async (req, res) => {
    const scope = await resolveClassroomSupervisorScope(req.authUser!);
    res.json({ interventions: await SchoolInterventionModel.find(classroomScopeFilter(scope)).sort({ createdAt: -1 }).limit(100).lean() });
  }));

  classroomRouter.post("/supervisor/interventions", requireAuth, requireRole(["admin", "supervisor"]), asyncHandler(async (req, res) => {
    const payload = interventionSchema.parse(req.body); const scope = await resolveClassroomSupervisorScope(req.authUser!);
    ...
    if (!(await resolveSchoolEntitlement(payload.schoolId, "INTERVENTION_CENTER")).allowed) return res.status(StatusCodes.FORBIDDEN).json({ message: "Intervention Center is not enabled for this school" });
    if (payload.classId && !(await GroupModel.findOne({ _id: payload.classId, type: "CLASS", parentId: payload.schoolId }).lean())) return res.status(StatusCodes.BAD_REQUEST).json({ message: "Class does not belong to this school" });
    ...
    const studyPlans = await Promise.all(students.map(async (student: any) => {
      ...
      return StudyPlanModel.create({ id: `school_intervention_${studentId}_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`, userId: studentId,
        name: `خطة علاج مهارة ${payload.skillId}`, pathId: String((path as any).id || (path as any)._id), subjectIds: [], courseIds: [],
        startDate: dateKey(now), endDate: dateKey(end), skipCompletedQuizzes: true, offDays: [], dailyMinutes: payload.dailyMinutes, preferredStartTime: "17:00", status: "active" });
    }));
    const intervention = await SchoolInterventionModel.create({ schoolId: payload.schoolId, classId: payload.classId, skillId: payload.skillId,
      targetStudentIds: targetIds, actionType: "study_plan", actionRef: String((path as any).id || (path as any)._id), assignedBy: req.authUser!.id,
      followUpAt: payload.followUpAt ? new Date(payload.followUpAt) : null, remediationThreshold: payload.remediationThreshold, minimumEvidence: payload.minimumEvidence, baseline });
