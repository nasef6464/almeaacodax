...
/**
 * POST /api/operations/migrate/quiz-kind
 * تحديث حقل quizKind في الاختبارات القديمة التي لا تملكه.
 * apply=false → dry-run | apply=true → تطبيق فعلي
 */
operationsRouter.post("/migrate/quiz-kind", requireAuth, requireRole(["admin"]), async (req, res, next) => {
  try {
    const apply = req.body?.apply === true;

    const legacy = await QuizModel.find({ quizKind: { $exists: false } })
      .select("_id id type placement showInTraining showInMock mockExam")
      .lean();

    const batches: Record<string, string[]> = { mock: [], drill: [], test: [] };
    ...
    for (const quiz of legacy) {
      ...
      const quizType = String((quiz as any).type || "quiz");
      ...
      } else if (quizType === "bank" || placement === "training" || (showInTraining === true && showInMock === false)) {
    ...
    const summary = {
      total: legacy.length,
      mock: batches.mock.length,
      drill: batches.drill.length,
      test: batches.test.length,
      applied: false,
      modifiedCount: 0,
    };

    if (!apply) {
      return res.json({ ...summary, message: "Dry-run: pass apply=true to apply changes" });
    }
    ...
    const modifiedCount = results.reduce((sum, r) => sum + (r as any).modifiedCount, 0);
    return res.json({ ...summary, applied: true, modifiedCount });
  } catch (error) {
    next(error);
  }
});
