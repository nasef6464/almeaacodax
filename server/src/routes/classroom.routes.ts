...
import {
  classroomQuestionVisibilityFilter,
  normalizeQuestionIds,
} from "../modules/schools/application/classroomQuestionAccess.js";
import {
  canMutateClassroomQuestions,
  canPublishClassroom,
  canStudentJoinClassroom,
  isDuplicateLiveSessionError,
  type ClassroomSessionStatus,
} from "../modules/schools/application/classroomLifecycle.js";
...
const questionLookupFilter = (questionIds: string[], schoolId: string, actorId: string) => {
...
const mapQuestionsByAnyId = (questions: any[]) => {
...
async function loadApprovedVisibleQuestions(questionIds: string[], schoolId: string, actorId: string) {
  const ids = normalizeQuestionIds(questionIds);
  const questions = await QuestionModel.find(questionLookupFilter(ids, schoolId, actorId)).lean();
  const byId = mapQuestionsByAnyId(questions as any[]);
...
async function ensureTeacherSchoolAccess(actor: any, schoolId: string) {
...
async function finalizeClassroomSession(session: any) {
  ...
  emitClassroomEventToClass(String(session.classId), "session:ended", { sessionId: sId });
  return report;
}
...
classroomRouter.post("/sessions", requireAuth, requireRole(["teacher", "admin"]), asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res.status(StatusCodes.CREATED).json({ sessionId: sessionId(session), pin, status: session.status });
  } catch (error) {
...
classroomRouter.get("/templates", requireAuth, requireRole(["teacher", "admin"]), asyncHandler(async (req, res) => {
  ...
  res.json({
    templates: templates.map((template: any) => ({
      ...
      challengeIds: template.challengeIds || [],
      badge: template.badge || "حزمة مخصصة للمعلم",
...
classroomRouter.post("/templates", requireAuth, requireRole(["teacher", "admin"]), asyncHandler(async (req, res) => {
  ...
  const canonicalChallengeIds = challengeIds
    .map((id) => {
      const requestedIndex = questionIds.indexOf(id);
      return requestedIndex >= 0 ? canonicalIds[requestedIndex] : null;
    })
...
classroomRouter.post("/templates/:id/delete", requireAuth, requireRole(["teacher", "admin"]), asyncHandler(async (req, res) => {
  ...
  if (req.authUser!.role !== "admin" && String(template.teacherId) !== req.authUser!.id) {
    return res.status(StatusCodes.FORBIDDEN).json({ message: "Template access denied" });
  }
...
classroomRouter.get("/teacher/history", requireAuth, requireRole(["teacher", "school_admin", "admin"]), asyncHandler(async (req, res) => {
  ...
  if (req.authUser!.role === "teacher") {
    ...
    if (requestedSchoolId) {
      const allowed = await hasActiveSchoolRole(req.authUser!, requestedSchoolId, "teacher");
      if (!allowed) return res.status(StatusCodes.FORBIDDEN).json({ message: "School history access denied" });
    ...
    const capability = await requireSchoolDirectorCapability(req.authUser!.id, schoolId, "SCHOOL_SMART_CLASSROOM_VIEW", "SMART_CLASSROOM");
    if (!capability) return res.status(StatusCodes.FORBIDDEN).json({ message: "School history access denied" });
...
classroomRouter.post("/sessions/:id/publish/:index", requireAuth, requireRole(["teacher", "admin"]), asyncHandler(async (req, res) => {
  ...
  if (!canPublishClassroom(session.status as ClassroomSessionStatus)) {
    return res.status(StatusCodes.CONFLICT).json({ message: "Session lifecycle does not allow publishing" });
  }
...
classroomRouter.post("/sessions/:id/join", requireAuth, sensitiveActionRateLimiter, asyncHandler(async (req, res) => {
  const payload = joinSchema.parse(req.body);
  const session = await ClassroomSessionModel.findById(req.params.id).lean() as any;
  if (!session || !canStudentJoinClassroom(session.status as ClassroomSessionStatus) || session.pinExpiresAt < new Date() || hashPin(payload.pin) !== session.pinHash) {
...
classroomRouter.get("/sessions/:id/current", requireAuth, asyncHandler(async (req, res) => {
  ...
  const currentQ = safeQuestions.find((question: any) => question.index === session.activeQuestionIndex) || safeQuestions[0] || null;
...
classroomRouter.post("/sessions/:id/end", requireAuth, requireRole(["teacher", "admin"]), asyncHandler(async (req, res) => {
  ...
  const report = await finalizeClassroomSession(session);
  res.json({ report });
}));
...
classroomRouter.post("/sessions/:id/append-questions", requireAuth, requireRole(["teacher", "admin"]), asyncHandler(async (req, res) => {
  ...
  const session = await ClassroomSessionModel.findById(req.params.id);
  if (!session || !canMutateClassroomQuestions(session.status as ClassroomSessionStatus)) {
    return res.status(StatusCodes.CONFLICT).json({ message: "الحصة لا تسمح بإضافة أسئلة في حالتها الحالية" });
  }
  if (req.authUser!.role !== "admin" && String(session.teacherId) !== req.authUser!.id) {
  ...
  if (!snapshots || snapshots.length !== requestedIds.length) {
    return res.status(StatusCodes.BAD_REQUEST).json({ message: "بعض الأسئلة غير موجودة أو غير معتمدة أو خارج نطاق المدرسة" });
  }
  ...
  if (trulyNewSnapshots.length === 0) {
    return res.status(StatusCodes.CONFLICT).json({ message: "كل الأسئلة المحددة موجودة بالفعل داخل الحصة" });
  }
  ...
  if (payload.autoPublishFirst) {
    ...
    const firstIndex = session.questionSnapshots.findIndex((question: any) => String(question.questionId) === newQuestionIds[0]);
    session.status = "live";
