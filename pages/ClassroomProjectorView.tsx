...
export const ClassroomProjectorView: React.FC = () => {
  ...
  const [competition, setCompetition] = useState<{ leaderboard?: CompetitionEntry[] } | null>(null);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const [challenge, competitionResult] = await Promise.all([
        api.get<ChallengeState>(`/classroom/sessions/${encodeURIComponent(sessionId)}/challenge-state`).catch(() => null),
        api.get<{ leaderboard?: CompetitionEntry[] }>(`/classroom/sessions/${encodeURIComponent(sessionId)}/competition`).catch(() => null),
      ]);
      setChallengeState(challenge);
      setCompetition(competitionResult);
      if (!challenge?.competitionEnabled) setShowPodium(false);
    } catch {
      setData(null);
  ...
  const challengeExpired = Boolean(challengeState?.competitionEnabled && challengeSeconds === 0);
  const podium = (competition?.leaderboard || []).slice(0, 3);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans select-none" dir="rtl">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-1.5 overflow-x-auto py-1">
            {questions.map((question: any, index: number) => {
              ...
              return (
                <button key={question.questionId || index} type="button" onClick={() => { setSelectedQuestionIdx(index); setShowExplanation(false); }} className={`flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-black transition-all ${isSelected ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}>
          ...
          <div className="flex items-center gap-3">
            {challengeState?.competitionEnabled && challengeSeconds !== null && (
              <div className={`flex items-center gap-2 rounded-xl border px-3.5 py-1.5 text-xs font-black ${challengeExpired ? 'border-rose-500/50 bg-rose-950/50 text-rose-300' : 'border-amber-500/40 bg-amber-950/40 text-amber-300'}`}>
                <Clock size={16} /><span className="font-mono text-base">{challengeSeconds}s</span>
              </div>
            )}
            {challengeState?.competitionEnabled && (
              <button type="button" onClick={() => setShowPodium((value) => !value)} className="flex items-center gap-1.5 rounded-xl border border-amber-500/40 bg-amber-500/10 px-3.5 py-2 text-xs font-black text-amber-300 hover:bg-amber-500/20">
                <Trophy size={16} /> {showPodium ? 'إخفاء منصة التحدي' : 'عرض منصة التحدي'}
              </button>
            )}
            <div className="flex items-center gap-2 rounded-xl bg-slate-800/80 px-3.5 py-1.5 text-xs font-bold text-slate-300 border border-slate-700">
              <Users size={16} className="text-indigo-400" /><span>إجابات الطلاب:</span><span className="font-mono text-base font-black text-emerald-400">{totalResponses}</span>
            </div>
      ...
      <main className="flex-1 p-6 sm:p-10 max-w-7xl mx-auto w-full flex flex-col justify-between">
        {showPodium && challengeState?.competitionEnabled && (
          <section className="mb-6 rounded-3xl border border-amber-500/40 bg-gradient-to-br from-amber-950/60 via-slate-900 to-indigo-950/50 p-6 shadow-2xl">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2"><Crown size={24} className="text-amber-400" /><div><h2 className="text-xl font-black text-amber-200">منصة التحدي</h2><p className="text-xs text-amber-300/80">100 نقطة لكل إجابة صحيحة · بدون Bonus سرعة</p></div></div>
              <span className={`rounded-full px-3 py-1 text-xs font-black ${challengeExpired ? 'bg-rose-500/20 text-rose-300' : 'bg-emerald-500/20 text-emerald-300'}`}>{challengeExpired ? 'انتهى الوقت' : 'التحدي مباشر'}</span>
            </div>
            {podium.length === 0 ? (
              <p className="mt-5 text-center text-sm font-bold text-slate-400">بانتظار أولى الإجابات الصحيحة.</p>
            ) : (
              <div className="mt-6 grid gap-4 sm:grid-cols-3">
                {podium.map((entry) => (
                  <div key={entry.studentId} className="rounded-2xl border border-amber-500/30 bg-slate-900/80 p-5 text-center">
                    <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-amber-500 text-xl font-black text-slate-950">{entry.rank}</div>
                    <h3 className="mt-3 truncate text-lg font-black text-white">{entry.name}</h3>
                    <div className="mt-2 text-3xl font-black text-amber-400">{entry.score}</div>
                    <p className="mt-1 text-xs font-bold text-slate-400">{entry.correct} صحيح · دقة {entry.accuracy}%</p>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-2.5">
                ...
                {isChallengeQuestion && <span className="flex items-center gap-1 rounded-xl bg-amber-500/20 px-3 py-1.5 text-xs font-black text-amber-300 border border-amber-500/30"><Flame size={14} /> تحدي سرعة</span>}
              </div>
              <button type="button" onClick={() => setShowExplanation(!showExplanation)} className={`flex items-center gap-1.5 rounded-xl px-4 py-2 text-xs font-black transition-all shadow-md active:scale-95 ${showExplanation ? 'bg-amber-500 text-slate-950' : 'border border-slate-700 bg-slate-800 text-amber-300'}`}>
                {showExplanation ? <EyeOff size={16} /> : <Eye size={16} />}{showExplanation ? 'إخفاء الشرح والحل النموذجي' : 'إظهار الشرح والحل النموذجي'}
              </button>
            </div>
                  ...
                  return (
                    <div key={index} className={`relative overflow-hidden rounded-2xl border p-5 transition-all ${isCorrect ? 'border-emerald-500 bg-emerald-950/40 shadow-xl shadow-emerald-500/10' : isCommonMistake ? 'border-rose-500/80 bg-rose-950/30 shadow-lg' : 'border-slate-800 bg-slate-850/70 hover:border-slate-700'}`}>
                      ...
                      <div className="relative z-10 flex items-center justify-between gap-3">
                        <div className="flex items-center gap-3.5"><span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-base font-black ${isCorrect ? 'bg-emerald-500 text-slate-950' : 'bg-slate-800 text-slate-200 border border-slate-700'}`}>{OPTION_LETTERS[index] || index + 1}</span><span className="text-lg sm:text-xl font-bold text-white leading-normal">{option}</span></div>
                        <div className="flex items-center gap-2 text-left shrink-0">{isCorrect && <span className="flex items-center gap-1 rounded-lg bg-emerald-500/20 px-2 py-1 text-xs font-black text-emerald-400 border border-emerald-500/30"><CheckCircle2 size={14} /> صحيح</span>}<span className="font-mono text-lg font-black text-slate-300">{percent}%</span><span className="text-xs text-slate-500">({count})</span></div>
                      </div>
                    </div>
                  );
                })}
            ...
            {analytics.maxWrongOption && (
              <div className="rounded-2xl border border-rose-500/40 bg-rose-950/20 p-5 text-rose-200 flex items-start gap-3 shadow-lg"><AlertTriangle size={24} className="text-rose-400 shrink-0 mt-0.5" /><div><h4 className="text-base font-black text-rose-300">تشخيص الفصل: خطأ شائع ملحوظ في هذا السؤال</h4><p className="text-sm mt-1 text-rose-200 leading-relaxed">نسبة <strong className="text-rose-100 font-black">{analytics.maxWrongOption.percent}%</strong> من الطلاب اختاروا البديل ({analytics.maxWrongOption.letter})، وهو أكثر خيار خاطئ تم اختياره ({analytics.maxWrongOption.count} طالباً).</p></div></div>
            )}
            ...
            {showExplanation && (
              <div className="rounded-3xl border border-amber-500/40 bg-slate-900 p-6 sm:p-8 shadow-2xl text-right animate-in fade-in slide-in-from-top-4 duration-300">
                <div className="flex items-center justify-between border-b border-slate-800 pb-4"><div className="flex items-center gap-2 text-amber-400 font-black text-base"><Sparkles size={20} /><span>المهارة المستهدفة والشرح النموذجي</span></div>{currentQ.skillIds?.length > 0 && <span className="rounded-xl bg-indigo-500/20 px-3 py-1 text-xs font-black text-indigo-300 border border-indigo-500/30">مهارة: {currentQ.skillIds.join(' • ')}</span>}</div>
                <div className="mt-5 text-base sm:text-lg text-slate-200 leading-relaxed">{currentQ.explanation ? <div className="whitespace-pre-line font-medium">{currentQ.explanation}</div> : <p className="text-slate-400 italic">لا يوجد شرح مسجل مسبقاً لهذا السؤال في بنك الأسئلة.</p>}</div>
              </div>
            )}
          </div>
        )}
